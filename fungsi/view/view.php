...
class view
{
    ...
    public function id_barang($offset, $itemsPerPage)
    {
        $sql = "SELECT * FROM barang ORDER BY id DESC LIMIT :offset, :itemsPerPage";
        $row = $this->db->prepare($sql);
        $row->bindValue(':offset', $offset, PDO::PARAM_INT);
        $row->bindValue(':itemsPerPage', $itemsPerPage, PDO::PARAM_INT);
        $row->execute();
        $hasil = $row->fetchAll();
        return $hasil;
    }

    public function stok($offset, $itemsPerPage)
    {
        $sql = "SELECT * FROM barang WHERE stok <= 3 ORDER BY id DESC LIMIT :offset, :itemsPerPage";
        $row = $this->db->prepare($sql);
        $row->bindValue(':offset', $offset, PDO::PARAM_INT);
        $row->bindValue(':itemsPerPage', $itemsPerPage, PDO::PARAM_INT);
        $row->execute();
