<?php
    ...
    if(isset($_POST['proses'])) {
        ...
        if ($jum > 0) {
            ...
            // Periksa role user (admin atau kasir)
            if ($hasil['role'] == 'admin') {
                $_SESSION['admin'] = $hasil;
                echo '<script>alert("Login Sukses sebagai Admin");window.location="index.php"</script>';
            } elseif ($hasil['role'] == 'kasir') {
                $_SESSION['kasir'] = $hasil;
                echo '<script>alert("Login Sukses sebagai Kasir");window.location="index.php"</script>';
            } else {
                echo '<script>alert("Role tidak dikenali");history.go(-1);</script>';
            }

        } else {
            echo '<script>alert("Login Gagal, periksa user dan password");history.go(-1);</script>';
        }
    }
?>
